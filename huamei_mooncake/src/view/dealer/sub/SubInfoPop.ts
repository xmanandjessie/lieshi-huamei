class SubInfoPop extends PopView
{
	private btnAdd:CustomImage;
	private btnReset:CustomImage;
	...
	public setData(data):void
	{
		...
		var bg = new CustomImage("resource/assets/asyn/sub/sub_info_bg.png",true,()=>{
		...
		var bg1 = new CustomImage("resource/assets/asyn/sub/input_bg.png",true,()=>{
			bg1.x = 305;
			bg1.y = 215;
			this.txtName.x = bg1.x + 20;
			this.txtName.y = bg1.y;
		});
		this.addChild(bg1);

		var bg3 = new CustomImage("resource/assets/asyn/sub/input_bg.png",true,()=>{
			bg3.x = 305;
			bg3.y = 350;
			this.txtPhone.x = bg3.x + 20;
			this.txtPhone.y = bg3.y;
		});
		this.addChild(bg3);
		...
		this.btnAdd = new CustomImage("resource/assets/asyn/sub/btn_update.png",true,()=>{
			this.btnAdd.x = 70;
			this.btnAdd.y = 455;
		});
		this.addChild(this.btnAdd);

		this.btnReset = new CustomImage("resource/assets/asyn/sub/btn_reset.png",true,()=>{
			this.btnReset.x = StageUtils.SW - this.btnReset.width - 70;
			this.btnReset.y = 455;
		});
		...
		this.btnAdd.addEventListener(egret.TouchEvent.TOUCH_TAP,()=>{
			...
			var status = this.txtStatus.text == "启用" ? 1 : 0;
			...
			{
				Global.post("cs/msa",{subaccountkey:this.data.subaccountkey,name:name,phone:phone,status:status}).then((data)=>{
					Message.show("修改成功!");
				}).catch((error)=>{
					Message.show("修改失败!");
				});
		...
		this.btnReset.addEventListener(egret.TouchEvent.TOUCH_TAP,()=>{
			if(confirm("密码重置后，该账号下次登录时需使用新密码，确定重置吗？"))
			{
				Global.post("cs/rsap",{subaccountkey:this.data.subaccountkey}).then((data:any)=>{
					Message.show("重置成功!");
					this.txtReset = new HTMLText();
					this.txtReset.setValue("长按复制该子账号的新密码："+data.pass);
					this.txtReset.setPosition(0,550,StageUtils.SW,80);
					this.txtReset.fontSize = 20;
					this.txtReset.show();
				}).catch((error)=>{
					Message.show("重置失败!");
				});
